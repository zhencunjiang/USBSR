...
class downsampler(nn.Sequential):
    def __init__(self, conv, n_feat, bn=False, act=False, bias=True):
        ...
        m.append(nn.PixelUnshuffle(4))
        m.append(conv(1024, n_feat, 3, bias))
...
class EDown(nn.Module):
    def __init__(self, nb=4, nf=64, res_scale=0.1, upscale=2, conv=default_conv):
        super(EDown, self).__init__()
        ...
        # define tail module
        m_tail = [
            downsampler(conv, n_feats, act=False),
